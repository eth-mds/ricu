...
```{asis, echo = !srcs_avail(demo)}
> Note: Code in this vignette requires that datasets `r paste1(demo)` are available, which can be installed by running
>
>     install.packages(
...
>     )
>
> As long as the corresponding datasets are not accessible, certain code blocks are not evaluated. The full version of this vignette is available from [CRAN](https://CRAN.R-project.org/package=ricu/vignettes/uom.html).
```
  ...
  itm_load_report_empty <- quote({
    ...
    if (!nrow(res) && !inherits(x, "nul_itm")) {
      ...
      while (!identical(.GlobalEnv, env <- parent.frame(i)) &&
             !inherits(cncpt <- get0("x", envir = env))) {
        i <- i + 1
      }
...
```{asis, echo = !srcs_avail(full)}
> Note: The following code blocks are run using demo (`r paste1(demo)`) instead of full datasets (`r paste1(full)`) and therefore might be less usefull. For data set up, please consult the manual at `?attach_src`. The full version of this vignette is available from [CRAN](https://CRAN.R-project.org/package=ricu/vignettes/uom.html).
```
...
dat <- load_concepts(concepts, srcs)
