...
```{r sep-itms, eval = eval_next, include = FALSE}
...
dict <- load_dictionary(srcs, concepts)
...
itm_load_report_empty <- function(x, ...) {

  res <- do_itm_load(x, ...)

  if (!nrow(res) && !inherits(x, "nul_itm")) {
    add_empty <- list(name = get0("x", envir = parent.frame(5))$name,
                      id = x$ids, source = x$src, table = x$table)
...
dat <- mockthat::with_mock(
  do_itm_load = itm_load_report_empty,
  load_concepts(dict, srcs, merge = FALSE, verbose = FALSE)
)

empty_items <- data.table::rbindlist(empty_items)

if (length(empty_items)) {
  knitr::kable(empty_items, caption = paste("For the given concepts, the",
    "listed item IDs do not return any data when the respective tables and",
    "data sources are queried."))
}
```
