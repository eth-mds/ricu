...
title:
  ...
  plain: "ricu: R's interface for Intensive Care data"
...
abstract: >
  Providing computational infrastructure for handling diverse intensive care unit (ICU) datasets, the \proglang{R} package \pkg{ricu} enables writing dataset-agnostic analysis code, thereby facilitating multi-center training and validation of machine learning models. The package is designed with an emphasis on extensibility both to new datasets as well as clinical data concepts, and currently supports the loading of around 100 patient variables corresponding to a total of 319,402 ICU admissions from 4 data sources collected in Europe and the United States. By allowing for the addition of user-specified medical concepts and data sources the aim of \pkg{ricu} is to foster robust intensive care research, allowing the user to externally validate their method or conclusion with relative ease.
keywords:
  formatted: [medicine, intensive care, computational physiology]
  plain:     [medicine, intensive care, computational physiology]
preamble: >
  \usepackage{amsmath}
vignette: >
...
Given the somewhat narrow focus of the targeted datasets, combined with the fact that in some cases data is even extracted from identical patient care management systems, it may come as a surprise as to how heterogeneous the resulting datasets are. In MIMIC-III and HiRID, for example, time-stamps are reported as absolute times (albeit randomly shifted due to data privacy concerns), whereas eICU and AUMC use relative times (with origins being admission times). Furthermore, while relative times in eICU are reported in minutes, AUMC uses millisecond resolution. Another example, involves different types of patient identifiers and their use among datasets. Common to all is the notion on an ICU admission ID, but apart from that, the amount of available information varies. While ICU (and hospital) readmissions for a given patient can be identified in some, this is not possible in other datasets. Furthermore, use of identifier systems might not be consistent over tables, table layouts vary (*long* versus *wide*) and data organization in general is far from consistent over datasets.

The following sections serve to introduce \pkg{ricu} by first giving an overview of particularities of the four default dataset (section \ref{ready-to-use-datasets}), followed by some implementation details on how datasets are stored and how external data sources can be made accessible by \pkg{ricu} (section \ref{implementation-details}). The next section is concerned with how clinical data concepts are can be represented and loaded into memory for further analysis (section \ref{data-concepts}), wrapped up by a section containing two toy examples on how to use \pkg{ricu} to investigate actual clinical questions (section \ref{examples}).
...
Closing this section of introducing datasets supported by \pkg{ricu} is a table highlighting some of the major similarities and distinguishing features among the four data sources.

```{r src-overview, eval = srcs_avail(c("mimic", "eicu", "hirid", "aumc")), echo = FALSE, results = "asis"}
as_quant <- function(x) {
  if (identical(length(x), 0L)) return("-")
  res <- format(quantile(x, probs = seq(0.25, 0.75, 0.25), na.rm = TRUE),
                TRUE, 3)
  paste0(res[2L], " (", res[1L], " - ", res[3L], ")")
}

big_mark <- function(x) formatC(x, big.mark = ",", format = "d")

summarize <- function(src) {
  los_icu <- load_concepts("los_icu", src, verbose = FALSE)
  hosp_len <- load_concepts("los_hosp", src, verbose = FALSE)
  c(`Number of tables` = big_mark(length(as_src_env(src))),
    `Number of ICU admissions` = big_mark(nrow(los_icu)),
    `ICU stay length [days]` = as_quant(data_col(los_icu)),
    `Hospital stay length [days]` = as_quant(data_col(hosp_len))
  )
}

srcs <- c("MIMIC", "eICU", "HiRID", "AUMC")
summ <- vapply(tolower(srcs), summarize, character(4L))
knitr::kable(summ, format = "latex")
```
...
While `id_var`, `index_var` and `time_vars` are used to provided sensible defaults to functions used for general data loading (section \ref{data-loading}), `unit_var`, `val_var`, as well as potential user-defined defaults are only used in concept loading (see section \ref{clinical-concepts}) and therefore need not be prioritized when integrating new data sources until data concepts have been mapped.
...
One of the key components of \pkg{ricu} is a scheme for specifying how to retrieve data corresponding pre-defined clinical concepts from a given data source, in turn enabling dataset agnostic code for analysis. Heart rate, for example can be loaded using the `hr` concept as
...
In order to extend the current concept library both to new datasets and new concepts, further JSON files can be incorporated by adding their paths to `RICU_CONFIG_PATH`. Concepts with names that already exists are only used for their `sources` entries, such that `hr` for `aumc` was specified as
...
Ventilation is represented by several concepts: a ventilation indicator variable (`vent_ind`), as well as ventilation durations (`vent_dur`) are constructed from start and events (`vent_start` and `vent_end`). This includes any kind of mechanical ventilcation (invasive via an endotracheal or tracheostomy tube), as well as non-invasive ventilation via face or nasal masks. In line with other concepts belonging to this group, the current state is far from being comprehensive and expansion to further ventilation parameters is desirable.

The singular concept addressing antibiotics (`abx`) returns an indicator signaling whenever an antibiotic was administered. This includes any route of administration (intravenous, oral, topical, etc.) and does neither report dosage, nor active ingredient. Finally, vasopressor administration again is reported by several concepts representing different vasoactive drugs (including dopamine, dobutamine, epinephrine, noreponephrine), as well as different administration aspects such as rate, duration (and for use in SOFA scoring, rate administered for at least 60 minutes).
...
In its current form, \pkg{ricu} is not yet a comprehensive solution for ICU data analysis. More work is required expanding the library of clinical concepts, as well as integrating new data sources as they inevitably will become available. Future changes to some architectural features might very well become necessary as implicit assumptions on structuring of datasets might no longer hold for newly encountered data resources. Considerable effort has been invested into extensibility but naturally, some design decisions have been made exclusively on the basis of currently available datasets.

But given the lack of efforts aiming to co-integrate multiple data sources, paired with the obvious benefits of having available a platform facilitating external validation, as well as training on multiple data sources, it is our hope that the initial release of \pkg{ricu} may provide a starting point in an iterative process toward such a goal.

- extensibility: best effort, but unforseen limitations of new data might require some changes
- more work required for treatement-related concepts
- flexibility offered by ricu comes at a cost: more involved concepts might run into issues for non-hourly interval sizes
-->
