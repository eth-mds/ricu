...
1. Download and install the \pkg{ricu} package:

    * install \pkg{ricu} via CRAN with `install.packages("ricu")`

    * alternatively, to install from Github use `install_github("eth-mds/ricu")` (of the \pkg{remotes} package)

1. Get access to datasets and run setup:

    * get access to MIMIC-III, eICU and HiRID via [Physionet](https://physionet.org/register/)

    * get access to AUMCdb via [Amsterdam Medical Data Science Website](https://amsterdammedicaldatascience.nl/#amsterdamumcdb)

    * set `RICU_PHYSIONET_USER`, `RICU_PHYSIONET_PASS` and `RICU_AUMC_TOKEN` environment variables using the obtained information

    * run `setup_data_src()` and wait for data download and conversion

    A more detailed description of the datasets and the setup process is given in Section \ref{data-sources}. There is also a separate [vignette to help the user get started](https://CRAN.R-project.org/package=ricu/vignettes/ricu.html).

1. Use the main `load_concepts()` function to load 100+ available concepts:

    * for example, run `load_concepts(c("age", "glu"), "mimic")` to obtain data on glucose and age concepts in the MIMIC-III dataset

    * see `?load_concepts` for more details

    A more detailed description of available concepts and data loading is discussed in Section \ref{data-concepts}.

1. Add your own concepts:

    * although some commonly appearing concepts are already available (such as demographics, vitals and laboratory values), the user might be interested in adding their own, custom concepts
...
1. Add your own dataset:
    ...
    *  the user might be interested in analyzing their own dataset, or using it jointly with the four large in-built datasets in \pkg{ricu}
