...
load_difftime("admissions", "mimic_demo", subject_id > 44000,
              cols = c("hadm_id", "admittime", "dischtime"))[]
```
The function `load_difftime()` is expected to return timestamps as base R `difftime` vectors (using `mins` as time unit). The argument `id_hint` can be used to specify a preferred ID system, but if not available in specified raw table, `load_difftime()` will return data using the ID system with highest cardinality. In the above example, if `icustay_id` were requested, data would be returned using `hadm_id`, because `icustay_id` column is not available in the table, and the cardinality of `hadm_id` is higher than that of `subject_id` (there are always more unique hospital stay than unique patients). However, a `subject_id` request would be honored, as this identifier is available as a column in the `admissions` table.
...
```
\drago{for some reason I do not get any output here?}
