...
if (!srcs_avail(srcs)) {
  ...
  msg <- paste(
    ...
    "`?setup_src_data`. The full version of this vignette is available from",
...
demo_src_avail <- function(x, msg = FALSE) {
  ...
  if (srcs_avail(demo)) {
    ...
    txt <- paste(
      "Note: The following code chunk(s) are intended to be run using data",
      "from", paste0("`", srcs, "`", collapse = ", "), "which is not",
      "available. Alternatively, data from",
      paste0("`", demo, "`", collapse = ", "), "is used instead."
    )
...
```{r mimic_demo-tbls, eval = only_demo("mimic", TRUE), include = only_demo("mimic")}
mimic_demo
```
...
```{r eicu_demo-tbls, eval = only_demo("eicu", TRUE), include = only_demo("eicu")}
eicu_demo
```
...
```{r src-overview, eval = srcs_avail(c("mimic", "eicu", "hirid", "aumc")) || demo_src_avail(c("mimic", "eicu"), msg = TRUE), echo = FALSE, results = "asis"}
as_quant <- function(x) {
...
dict <- load_dictionary(sub(" \\(demo\\)$", "", tolower(srcs)))
avai <- concept_availability(dict, include_rec = FALSE)
summ <- vapply(sub(" \\(demo\\)$", "", tolower(srcs)), summarize,
               character(10L), avai)
...
```{r src_sel, eval = demo_src_avail("mimic", msg = TRUE), echo = FALSE}
if (is_data_avail("mimic")) {
  src <- "mimic"
} else {
  src <- "mimic_demo"
}
...
```{r time_cox_model, eval = demo_src_avail("mimic"), cache = TRUE}
...
```{r cox_plot, eval = demo_src_avail("mimic"), echo = FALSE, warning = FALSE, message = FALSE, fig.width = 8}
theme_fp <- function(...) {
  theme_bw(...) +
...
```{r ins24, eval = demo_src_avail("mimic")}
...
```{r diab, eval = demo_src_avail("mimic")}
...
```{r diabetes_visualize, echo = FALSE, eval = demo_src_avail("mimic"), fig.height = 3}
